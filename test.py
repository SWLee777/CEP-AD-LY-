...
def test(args):
    ...
    for idx, items in enumerate(tqdm(test_dataloader)):
        ...
        with torch.no_grad():
            ...
            fg_score = compute_gradient_foreground(image, patch_size=14)  # [1369, 1]

            # SCA-enhanced DAP
            patch_feat_i = patch_features[1:, 0, :]  # [1369, 768]（batch_size=1）
            fg_score_i   = fg_score[:, 0]            # [1369]
            ...
            text_features = torch.cat([text_features_pos, text_features_neg])
            text_features_list = [text_features]
            ...
            vis_score_i = torch.sigmoid(prompt_learner.vis_cls_head(bias)).squeeze()
            # Adaptive fusion: global structural anomalies (missing part, bent) get
            # higher vis_score → its weight grows, amplifying global detection signal.
            # Texture anomalies get lower vis_score → weight falls back to baseline.
            w_vis = 0.05 + 0.05 * vis_score_i.detach()   # range [0.2, 0.4]
            w_rest = 1.0 - w_vis
            tmp = w_rest * (0.5 * text_probs + 0.5 * map_max_score) + w_vis * vis_score_i
            ...
            # Multi-scale fusion: spread weak signals around missing-component regions
            # so absence-type anomalies get elevated scores from their surroundings.
            anomaly_map_3  = F.avg_pool2d(anomaly_map, kernel_size=3,  stride=1, padding=1)
            anomaly_map_5  = F.avg_pool2d(anomaly_map, kernel_size=5,  stride=1, padding=2)
            anomaly_map = 0.5 * anomaly_map + 0.3 * anomaly_map_3 + 0.2 * anomaly_map_5
            anomaly_map = torch.Tensor(gaussian_filter(anomaly_map.detach().cpu(), sigma=args.sigma))

            results[cls_name[0]]['anomaly_maps'].append(anomaly_map)
            #visualizer(items['img_path'], anomaly_map.detach().cpu().numpy(), args.image_size, args.save_path, args.img_path, cls_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--img_path", type=str, default='./results/', help='path to save img')
